...
ensure_waro_cli() {
  ...
  if [[ -f "$source_dir/Cargo.toml" ]]; then
    ...
    echo "No se encontro source de WARO CLI en $source_dir." >&2
    exit 1
  fi
  if [[ ! -x "$cli" ]]; then
    needs_build="true"
    build_reason="missing_binary"
  elif [[ "$installed_rev" != "$source_rev" ]]; then
    needs_build="true"
